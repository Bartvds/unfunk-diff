...
module unfunk {
	/*
	 ...
	 diff output algorithm based on objectDiff's HTML printer
	 */
	...
	export class DiffFormatter {
		...
		constructor(public style:MiniStyle.Style, public maxWidth:number = 80) {
			if (maxWidth === 0) {
				this.maxWidth = 100;
			}
		}
		...
		public getStringDiff(actual:string, expected:string, padLength:number, padFirst:string[], leadSymbols:boolean = false):string {
			...
			//return new unfunk.StringDiffer(this).getWrapping(actual, expected, this.maxWidth, padLength, padFirst, leadSymbols);
